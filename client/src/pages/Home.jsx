...
import { getMemes, TIME_FRAMES, getTrendingTags } from "../services/memeService";
...
const Home = () => {
  ...
  const loadSavedMemes = async () => {
    ...
    const allData = await getMemes(1, 200, sort, timeFrame, selectedTag);
    const filtered = allData.memes.filter((m) => savedIds.includes(m._id));

    const validIds = filtered.map((m) => m._id);
    if (validIds.length !== savedIds.length) {
      setSavedMemes(validIds);
      localStorage.setItem("savedMemes", JSON.stringify(validIds));
    }
  ...
  const fetchMemes = async (reset = false) => {
    try {
      ...
      if (showSaved) {
        const savedIds = JSON.parse(localStorage.getItem("savedMemes") || "[]");
        const fullData = await getMemes(1, 100, sort, timeFrame, selectedTag);

        const filtered = fullData.memes.filter((m) => savedIds.includes(m._id));

        setMemes(filtered);
        setHasMore(false);
        setPage(1);
      } else {
        const response = await getMemes(newPage, 10, sort, timeFrame, selectedTag);
        ...
        // Clean saved IDs
        const validIds = response.memes.map((m) => m._id);
        const cleanedSaved = savedMemes.filter((id) => validIds.includes(id));

        if (cleanedSaved.length !== savedMemes.length) {
          setSavedMemes(cleanedSaved);
          localStorage.setItem("savedMemes", JSON.stringify(cleanedSaved));
        }
  ...
  const toggleSaveMeme = (memeId) => {
    let updated;

    if (savedMemes.includes(memeId)) {
      updated = savedMemes.filter((id) => id !== memeId);
      showToast("Meme removed from saved", "info");
    } else {
      updated = [...savedMemes, memeId];
        ...
        <form onSubmit={handleSearch} className="mb-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-black focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              autoComplete="off"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex flex-wrap gap-2 justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              aria-label={showSaved ? "Show All Memes" : "Show Saved Memes"}
            >
